...
pub struct NodeLink {
    ...
    // The text that we have for the node
    pub t: String,
    ...
    // Accessibility title or hover
    pub a: String,

    // Outbound Range Array
    pub o: String,
}
...
pub struct NodeList {
    ...
    // Elements and what type this is
    pub e: String,
...
#[derive(Debug, Default, Clone, Copy)]
pub struct Embed {
    // element value of our embed
    pub e: String,

    // u = 'url', the source url that is being embedded
    pub u: UrlString,

    // c = ‘content url’, the embed url (iframe url)
    pub c: UrlString,

    // x = 'x' or width, the native width of the embed
    pub x: number,

    // y = 'y' or height, the native height of the embed
    pub y: number,
}
...
#[doc(hidden)]
pub fn make_block(value: NodeValue, start_line: u32, start_column: usize) -> RTJson {
    RTJson {
        value: value,
        content: String::new(),
...
pub type RTJsonNode<'a> = Node<'a, RefCell<RTJson>>;

#[doc(hidden)]
pub fn last_child_is_open<'a>(node: &'a RTJsonNode<'a>) -> bool {
    node.last_child().map_or(false, |n| n.data.borrow().open)
}

#[doc(hidden)]
pub fn can_contain_type<'a>(node: &'a RTJsonNode<'a>, child: &NodeValue) -> bool {
...
#[doc(hidden)]
pub fn ends_with_blank_line<'a>(node: &'a RTJsonNode<'a>) -> bool {
...
#[doc(hidden)]
pub fn containing_block<'a>(node: &'a RTJsonNode<'a>) -> Option<&'a RTJsonNode<'a>> {
