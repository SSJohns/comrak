...
use nodes;
use nodes::{NodeValue, ListType, ListDelimType, NodeLink, RTJsonNode};
...
/// Formats an AST as RTJson, modified by the given options.
pub fn format_document<'a>(root: &'a RTJsonNode<'a>, options: &ComrakOptions) -> String {
    let mut f = RTJsonFormatter::new(root, options);
    f.format(root);
...
impl<'a, 'o> Write for RTJsonFormatter<'a, 'o> {
...
impl<'a, 'o> RTJsonFormatter<'a, 'o> {
    fn new(node: &'a RTJsonNode<'a>, options: &'o ComrakOptions) -> Self {
        RTJsonFormatter {
            node: node,
            options: options,
            v: vec![],
    ...
    fn format_node(&mut self, node: &'a RTJsonNode<'a>, entering: bool) -> bool {
        // self.node = node;
